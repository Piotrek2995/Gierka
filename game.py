...
class Game:
    ...
    def new(self):
        ...
        for plat in PLATFORM_LIST:
             p = Platform(*plat, 100, 20) # *plat explodes the tuple into arguments, plus width and height
             self.all_sprites.add(p)
             self.platforms.add(p)
             
        # Add a big ground platform for start
        p = Platform(0, HEIGHT - 40, WIDTH * 2, 40)
        self.all_sprites.add(p)
        self.platforms.add(p)
    ...
    def update(self):
        ...
        if self.player.rect.right > WIDTH / 2:
            ...
            while len(self.platforms) < 6:
                ...
                rightmost_plat = max(self.platforms, key=lambda x: x.rect.right)
                ...
                p.lifetime = random.randrange(4000, 7000) # Increased by 1s (was 3000-6000)
                
                self.platforms.add(p)
                self.all_sprites.add(p)
                
                # Chance to spawn items
                if random.randrange(100) < 15: # 15% chance for obstacle
                    o = Obstacle(p)
